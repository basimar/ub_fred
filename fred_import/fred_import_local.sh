#!/bin/bash
# Script zur Verarbeitung von angereicherten SE-Daten (FRED), die lokal auf dem Server liegen.
...
# Voraussetzungen: 
  ...
  heute=2020-06-27
  date=20200627
  file_data=out-local-$heute.txt
  file_load=out-local-$heute.seq
  file_log=out-local-$heute.log.gz
  file_log_extracted=out-local-$heute.log
  file_duplicates=duplicates-local-$heute.txt
  file_duplicates_mail=duplicates-local-mail-$heute.txt
  file_duplicates_bl=duplicates-local-blacklist-$heute.txt

  source $basedir/fred_import.conf

function quit {
   printf "FRED-Out-Datei vom ${heute}: http://aleph.unibas.ch/dirlist/u/dsv01/scripts/fred_import/input/${file_data}\n\nSystemnummernliste des Imports: http://aleph.unibas.ch/dirlist/u/alephe/scratch/out-${heute}.seq.doc_log\n\nDuplicates-Meldungen: http://aleph.unibas.ch/dirlist/u/dsv01/scripts/fred_import/input/${file_duplicates_mail}" | mailx -r @unibas.ch -s "FRED-Out-Datei vom ${heute} -- Achtung Fehler" $MAIL_EDV
    exit
}

function success {
   printf "FRED-Out-Datei vom ${heute}: http://aleph.unibas.ch/dirlist/u/dsv01/scripts/fred_import/input/${file_data}\n\nSystemnummernliste des Imports: http://aleph.unibas.ch/dirlist/u/alephe/scratch/out-${heute}.seq.doc_log\n\nDuplicates-Meldungen: http://aleph.unibas.ch/dirlist/u/dsv01/scripts/fred_import/input/${file_duplicates_mail}" | mailx -r @unibas.ch -s "FRED-Out-Datei vom ${heute}" $MAIL_EDV
    exit
}
